...
immutable struct JsName {
	...
	static JsName noPrefix(Symbol name) =>
		JsName(JsNameKind.none, name);
	static JsName local(Symbol name) =>
		JsName(JsNameKind.local, name);
	static JsName specialLocal(Symbol name) =>
		JsName(JsNameKind.specialLocal, name);
	static JsName temp(Symbol name, ushort index) =>
		JsName(JsNameKind.temp, name, some(index));
}
...
enum JsNameKind { none, function_, local, specialLocal, specSig, temp, type }
...
immutable struct JsMemberName {
	@safe @nogc pure nothrow:
	JsMemberNameKind kind;
	Symbol crowName;

	static JsMemberName noPrefix(Symbol name) =>
		JsMemberName(JsMemberNameKind.none, name);
	static JsMemberName enumMember(Symbol name) =>
		JsMemberName(JsMemberNameKind.enumMember, name);
	static JsMemberName recordField(Symbol name) =>
		JsMemberName(JsMemberNameKind.recordField, name);
	static JsMemberName special(Symbol name) =>
		JsMemberName(JsMemberNameKind.special, name);
	static JsMemberName sumTypeMethod(Symbol name) =>
		JsMemberName(JsMemberNameKind.sumTypeMethod, name);
	static JsMemberName unionConstructor(Symbol name) =>
		JsMemberName(JsMemberNameKind.unionConstructor, name);
	static JsMemberName unionMember(Symbol name) =>
		JsMemberName(JsMemberNameKind.unionMember, name);
}
enum JsMemberNameKind { none, enumMember, recordField, special, sumTypeMethod, unionConstructor, unionMember }
...
immutable struct JsDecl {
	Source source;
	JsDeclExported exported;
	JsName name;
	JsDeclKind kind;
}
enum JsDeclExported { private_, export_ }
...
immutable struct JsClassMember {
	Source source;
	JsClassMemberStatic isStatic;
	JsMemberName name;
	JsClassMemberKind kind;
}
enum JsClassMemberStatic { instance, static_ }
...
immutable struct JsVarDecl {
	JsVarDeclConst const_;
	JsDestructure destructure;
	Opt!(JsExpr*) initializer;
}
enum JsVarDeclConst { const_, let }
...
enum JsUnaryKind { await, bitwiseNot, not, typeof_, void_ }
...
JsExpr genAnd(ref Alloc alloc, in Source source, JsExpr arg0, JsExpr arg1) =>
	genBinary(alloc, source, JsBinaryKind.and, arg0, arg1);
...
JsExpr genAwait(ref Alloc alloc, in Source source, JsExpr arg) =>
	genUnary(alloc, source, JsUnaryKind.await, arg);
...
JsExpr genBinary(ref Alloc alloc, in Source source, JsBinaryKind kind, JsExpr arg0, JsExpr arg1) =>
...
JsExpr genBitwiseAnd(ref Alloc alloc, in Source source, JsExpr arg0, JsExpr arg1) =>
	genBinary(alloc, source, JsBinaryKind.bitwiseAnd, arg0, arg1);
JsExpr genBitwiseNot(ref Alloc alloc, in Source source, JsExpr arg) =>
	genUnary(alloc, source, JsUnaryKind.bitwiseNot, arg);
...
JsExpr genIn(ref Alloc alloc, in Source source, JsMemberName arg0, JsExpr arg1) =>
	genBinary(alloc, source, JsBinaryKind.in_, genStringFromMemberName(source, arg0), arg1);
JsExpr genInstanceof(ref Alloc alloc, in Source source, JsExpr arg0, JsExpr arg1) =>
	genBinary(alloc, source, JsBinaryKind.instanceof, arg0, arg1);
...
JsExpr genNot(ref Alloc alloc, in Source source, JsExpr arg) =>
	genUnary(alloc, source, JsUnaryKind.not, arg);
...
JsExpr genNotEqEq(ref Alloc alloc, in Source source, JsExpr left, JsExpr right) =>
	genBinary(alloc, source, JsBinaryKind.notEqEq, left, right);
...
JsExpr genOr(ref Alloc alloc, in Source source, JsExpr arg0, JsExpr arg1) =>
	genBinary(alloc, source, JsBinaryKind.or, arg0, arg1);
JsExpr genPlus(ref Alloc alloc, in Source source, JsExpr arg0, JsExpr arg1) =>
	genBinary(alloc, source, JsBinaryKind.plus, arg0, arg1);
...
JsExpr genTypeof(ref Alloc alloc, in Source source, JsExpr arg) =>
	genUnary(alloc, source, JsUnaryKind.typeof_, arg);
JsExpr genEqEqEq(ref Alloc alloc, in Source source, JsExpr a, JsExpr b) =>
	genBinary(alloc, source, JsBinaryKind.eqEqEq, a, b);
JsExpr genLess(ref Alloc alloc, in Source source, JsExpr a, JsExpr b) =>
	genBinary(alloc, source, JsBinaryKind.less, a, b);
...
JsStatement genVarDecl(in Source source, JsVarDeclConst const_, JsDestructure destructure, Opt!(JsExpr*) initializer) =>
	JsStatement(source, JsStatementKind(JsVarDecl(const_, destructure, initializer)));
...
JsStatement genConst(ref Alloc alloc, in Source source, JsDestructure destructure, JsExpr initializer) =>
	genVarDecl(source, JsVarDeclConst.const_, destructure, some(allocate(alloc, initializer)));
JsStatement genLet(in Source source, JsName name) =>
	genVarDecl(source, JsVarDeclConst.let, JsDestructure(name), none!(JsExpr*));
JsStatement genLet(ref Alloc alloc, in Source source, JsDestructure destructure, JsExpr initializer) =>
	genVarDecl(source, JsVarDeclConst.let, destructure, some(allocate(alloc, initializer)));
...
JsExpr genTimes(ref Alloc alloc, in Source source, JsExpr left, JsExpr right) =>
	genBinary(alloc, source, JsBinaryKind.times, left, right);
JsExpr genUnary(ref Alloc alloc, in Source source, JsUnaryKind kind, JsExpr arg) =>
	JsExpr(source, JsExprKind(JsUnaryExpr(kind, allocate(alloc, arg))));
JsExpr genUndefined(ref Alloc alloc, in Source source) =>
	JsExpr(source, JsExprKind(JsUnaryExpr(JsUnaryKind.void_, allocate(alloc, genNumber(source, 0)))));
...
JsClassMember genInstanceMethod(
	...
	genMethod(source, JsClassMemberStatic.instance, async, name, params, body_);
JsClassMember genStaticMethod(
	...
	genMethod(source, JsClassMemberStatic.static_, async, name, params, body_);
JsClassMember genInstanceMethod(
	ref Alloc alloc,
	in Source source,
	SyncOrAsync async,
...
JsClassMember genField(in Source source, JsClassMemberStatic static_, JsMemberName name, JsExpr value) =>
