...
ConcreteExpr concretizeFunPointer(
	...
	return has(called)
		? ConcreteExpr(type, range, ConcreteExprKind(Constant(ConstantFunPointer(force(called)))))
		: concretizeBogus(ctx, type, range);
}
	...
	if (partIndex == parts.length)
		...
		if (expectedType == valueType) {
			ConcreteExpr value = ConcreteExpr(valueType, range, isVoid(valueType)
				...
				: ConcreteExprKind(RecordFieldGetConcreteExpr(getTemp, partIndex)));
...
ConcreteExpr genIfOption(
	...
	ConcreteExpr(else_.type, range, ConcreteExprKind(
		allocate(alloc, MatchUnionConcreteExpr(
			option,
			integralValuesRange(2),
			newSmallArray!ConcreteMatchUnionCase(alloc, [
				ConcreteMatchUnionCase(none!(ConcreteLocal*), else_),
				ConcreteMatchUnionCase(then.rootLocal, then.expr)]),
			none!(ConcreteExpr*)))));
...
ConcreteExpr concretizeMatchEnum(
	...
	IntegralValues values = mapToIntegralValues!MatchEnumCase(a.cases, (ref MatchEnumCase x) =>
		x.member.value);
	...
	ConcreteExpr[] cases = map(ctx.alloc, values, (ref IntegralValue value) =>
		concretizeExpr(
			ctx, type, locals,
			mustFind!MatchEnumCase(a.cases, (ref MatchEnumCase x) => x.member.value == value).then));
	Opt!(ConcreteExpr*) else_ = optIf(has(a.else_), () =>
		allocate(ctx.alloc, concretizeExpr(ctx, type, locals, force(a.else_))));
	return genMatchEnumOrIntegral(ctx.alloc, type, range, matched, values, cases, else_);
}
...
ConcreteExpr concretizeMatchIntegral(
	...
	ConcreteExpr matched = concretizeExpr(ctx, locals, a.matched);
	IntegralValues values = mapToIntegralValues!MatchIntegralCase(a.cases, (ref MatchIntegralCase x) =>
		x.value);
	// TODO: If matched is a constant, just compile the relevant case
	ConcreteExpr[] cases = map(ctx.alloc, values, (ref IntegralValue value) =>
		concretizeExpr(
			ctx, type, locals,
			mustFind!MatchIntegralCase(a.cases, (ref MatchIntegralCase x) => x.value == value).then));
...
ConcreteExpr concretizeMatchSumType(
	...
	Opt!(ConcreteExpr*) else_ = optIf(has(a.else_), () =>
		allocate(ctx.alloc, concretizeExpr(ctx, type, locals, *force(a.else_))));
	return ConcreteExpr(type, range, ConcreteExprKind(
		allocate(ctx.alloc, MatchUnionConcreteExpr(matched, vc.values, vc.cases, else_))));
}
...
ValuesAndCases concretizeMatchSumTypeCases(
	...
	SmallArray!ConcreteMatchUnionCase concreteCases = map(ctx.alloc, values, (ref IntegralValue value) {
...
ConcreteMatchUnionCase toMatchUnionCase(RootLocalAndExpr x) =>
	ConcreteMatchUnionCase(x.rootLocal, x.expr);
	...
	a.matchWithPointers!ConcreteExpr(
		(Local* x) =>
			getLocal(locals, x).matchWithPointers!ConcreteExpr(
				(ConcreteLocal* local) =>
					// If it's a ref, leave it that way
					genLocalGet(range, local),
	...
	return a.condition.match!ConcreteExpr(
		(ref Expr x) {
			...
			return ConcreteExpr(type, range, ConcreteExprKind(allocate(ctx.alloc, a.isForbid
				? IfConcreteExpr(condition, throw_, after)
				: IfConcreteExpr(condition, after, throw_))));
		},
		(ref UnpackOption x) {
	...
	return a.matchWithPointers!ConcreteExpr(
		...
		(SeqExpr* x) {
			...
			return first.kind.isA!Constant
				? then
				: ConcreteExpr(type, range, ConcreteExprKind(
					allocate(ctx.alloc, SeqConcreteExpr(first, then))));
		},
		(ThrowExpr* x) =>
			ConcreteExpr(type, range, ConcreteExprKind(
				allocate(ctx.alloc, ThrowConcreteExpr(
	...
	args.match!ConstantsOrExprs(
		...
		(ConcreteExpr[] exprs) =>
			ConstantsOrExprs(newArray!ConcreteExpr(ctx.alloc, [
				ConcreteExpr(arrayType, range, ConcreteExprKind(CreateArrayConcreteExpr(exprs)))])));
...
ConcreteExpr concretizeTry(
	...
	return ConcreteExpr(type, range, ConcreteExprKind(allocate(ctx.alloc,
		TryConcreteExpr(tried, vc.values, vc.cases))));
}
...
ConcreteExpr concretizeTryLet(
	...
	*/
	...
	ConcreteMatchUnionCase catchExpr = toMatchUnionCase(
	...
	return ConcreteExpr(type, range, ConcreteExprKind(allocate(ctx.alloc,
		TryLetConcreteExpr(then.rootLocal, value, catchMemberIndex, catchExpr, then.expr))));
}
